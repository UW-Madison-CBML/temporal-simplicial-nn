...
1. :class:`SiSTSCCNLayer` -- **simultaneous** spatial+temporal, based on SiST-GNN
   (Roy-Shubhajit/SiST-GNN). The LSTM hidden ``h_p`` is the *temporal copy* of each cell;
...
SiST-GNN augmentation (variant 1), using SCCN's convention ``N[x, y]`` routes ``y -> x``,
...
def augment_adjacency(
    ...
    present_self_loop : bool, default=True
        ...
        updated feature (as in SiST-GNN's ``add_self_loops``).
...
class _TemporalSCCNBlockBase(torch.nn.Module):
    ...
    def init_state(self):
        """Return fresh ``(h_state, c_state)`` identity-keyed stores for a new sequence."""
    ...
    def _temporal_step(self, feats, keys, h_state, c_state):
        """Per-rank LSTM over time with identity-keyed alignment.

        Aligns the previous ``(h, c)`` to the current cell ordering (zeros for new cells),
        runs the per-rank ``LSTMCell`` on ``feats``, writes the new ``(h, c)`` back by
        identity (``detach=False`` -> gradients flow across windows), and returns the new
        per-rank hidden state ``{f"rank_{r}": (n_r, channels)}``.
        """
        out = {}
        for r in self.ranks:
            keys_r = keys[r]
    ...
    def forward(self, features, incidences, adjacencies, keys, h_state, c_state):
        raise NotImplementedError


class SiSTSCCNLayer(_TemporalSCCNBlockBase):
    """Simultaneous spatio-temporal SCCN block (SiST-GNN style augmentation).
    ...
    def forward(self, features, incidences, adjacencies, keys, h_state, c_state):
        ...
        h_p = self._temporal_step(features, keys, h_state, c_state)
...
class TemporalThenSpatialSCCNLayer(_TemporalSCCNBlockBase):
    ...
    def forward(self, features, incidences, adjacencies, keys, h_state, c_state):
        ...
        h = self._temporal_step(features, keys, h_state, c_state)
...
class SpatialThenTemporalSCCNLayer(_TemporalSCCNBlockBase):
    ...
    *un-augmented* matrices, then LSTM-encodes the spatial output over time.
    ...
    def forward(self, features, incidences, adjacencies, keys, h_state, c_state):
        ...
        return self._temporal_step(h, keys, h_state, c_state)
